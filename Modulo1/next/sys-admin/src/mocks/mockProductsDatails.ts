export const productsDetailsMocks = [
  {
    id: 1,
    name: "iPhone 15 Pro Max 256GB - Titânio Natural",
    price: 8999.99,
    originalPrice: 9999.99,
    images: [
      "https://images.unsplash.com/photo-1592750475338-74b7b21085ab?w=600&h=600&fit=crop",
      "https://images.unsplash.com/photo-1511707171634-5f897ff02aa9?w=600&h=600&fit=crop",
      "https://images.unsplash.com/photo-1510557880182-3d4d3cba35a5?w=600&h=600&fit=crop",
    ],
    rating: 4.8,
    discount: 10,
    reviews: 1250,
    category: "Smartphones",
    isNew: true,
    description:
      "O iPhone 15 Pro Max em Titânio Natural combina elegância e resistência em um design extraordinário. Com o chip A17 Pro e sistema de câmeras profissional, oferece performance excepcional para todas as suas necessidades.",
    features: [
      "Chip A17 Pro com GPU de 6 núcleos",
      "Sistema de câmeras Pro com teleobjetiva 5x",
      "Tela Super Retina XDR de 6,7 polegadas",
      "Bateria com duração de até 29 horas de reprodução de vídeo",
      "Resistente à água (IP68)",
      "Face ID para autenticação segura",
    ],
    specifications: {
      Tela: "6,7 polegadas Super Retina XDR",
      Processador: "Chip A17 Pro",
      Memória: "256GB",
      Câmera: "48MP Principal + 12MP Ultra Angular + 12MP Teleobjetiva",
      Bateria: "Até 29 horas de reprodução de vídeo",
      Sistema: "iOS 17",
    },
    shipping: "Entrega grátis para todo o Brasil",
    warranty: "1 ano de garantia Apple",
  },
];
